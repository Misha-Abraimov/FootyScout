  ...
  return (
    <article className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-5">
      ...
      <dl className="mt-7 grid grid-cols-3 gap-3">
        ...
        <div><dt className="text-xs text-[var(--muted)]">Expected</dt><dd className="metric-tabular mt-1 font-semibold">{formatPercent(expected)}</dd></div>
        <div><dt className="text-xs text-[var(--muted)]">Above</dt><dd className="metric-tabular mt-1 font-semibold text-[var(--accent-strong)]">{formatPercentagePoints(aboveExpected)}</dd></div>
      </dl>
